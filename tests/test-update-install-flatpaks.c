 ...
 * other remote. */
static void
test_update_install_flatpaks_in_repo_also_pull_runtimes_prioritize_source_remote (EosUpdaterFixture *fixture,
                                                                                  gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
  ...
  g_autoptr(GHashTable) flatpak_repo_infos = g_hash_table_new_full (g_str_hash,
                                                                    ...
                                                                    g_free,
                                                                    (GDestroyNotify) flatpak_repo_info_free);
  g_auto(GStrv) refspecs_in_repo = NULL;
  g_autoptr(GFile) updater_directory = NULL;
  g_autofree gchar *updater_directory_str = NULL;
  g_autofree gchar *keyid = get_keyid (fixture->gpg_home);
  g_autoptr(GFile) gpg_key_file = get_gpg_key_file_for_keyid (fixture->gpg_home, keyid);
  ...
  /* Assert that our runtime was pulled into the local repo */
  refspecs_in_repo = ostree_refspecs_in_installation_repo (flatpak_user_installation_dir,
                                                           &error);
  g_assert_no_error (error);

  g_assert_true (g_strv_contains ((const gchar * const *) refspecs_in_repo,
                                  "test-repo:runtime/org.test.Runtime/arch/stable"));
  g_assert_false (g_strv_contains ((const gchar * const *) refspecs_in_repo,
                                   "other-repo:runtime/org.test.Runtime/arch/stable"));
}

/* Insert a list of flatpaks to automatically install on the commit,
 * with both of the flatpaks using a runtime that we do not have installed.
 * That runtime will be available both in the source remote and in another
 * remote. We should take the runtime from the source remote of the first
 * flatpak in the list and not the other remote. */
static void
test_update_install_flatpaks_in_repo_also_pull_runtimes_first_dep_remote_wins (EosUpdaterFixture *fixture,
                                                                               gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
  FlatpakToInstall flatpaks_to_install[] = {
    { "install", "com.endlessm.TestInstallFlatpaksCollection", "test-repo", "org.test.Test", "stable", "app", FLATPAK_TO_INSTALL_FLAGS_NONE },
    { "install", "com.endlessm.TestInstallOtherFlatpaksCollection", "other-repo", "org.test.Other", "stable", "app", FLATPAK_TO_INSTALL_FLAGS_NONE }
  };
  g_autofree gchar *flatpak_user_installation = NULL;
  g_autoptr(GFile) flatpak_user_installation_dir = NULL;
  g_autoptr(GPtrArray) flatpak_install_infos = g_ptr_array_new_with_free_func ((GDestroyNotify) flatpak_install_info_free);
...
/* Insert a list of flatpaks to automatically install on the commit,
 * including a flatpak that has a dependency on a runtime that is not
 * yet installed. Pull the update, kill the network connection by
 * deleting the remote ostree repo and then simulate a reboot by running
 * eos-updater-flatpak-installer installer. Both the incoming package
 * and the dependency should be installed (in this case, dependency
 * resolution will deploy the runtime during the fetch phase such
 * that it will appear to be installed after the reboot). */
static void
test_update_deploy_dependency_runtime_flatpaks_on_reboot_no_network (EosUpdaterFixture *fixture,
                                                                     gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
  ...
  g_autofree gchar *flatpak_user_installation = NULL;
  g_autoptr(GFile) flatpak_user_installation_dir = NULL;
  g_autoptr(GFile) flatpak_remote_repositories = NULL;
  g_auto(GStrv) deployed_flatpaks = NULL;
  g_autofree gchar *deployment_repo_relative_path = g_build_filename ("sysroot", "ostree", "repo", NULL);
  g_autofree gchar *deployment_csum = NULL;
  g_autofree gchar *refspec = concat_refspec (default_remote_name, default_ref);
...
/* Insert a list of flatpaks to automatically install on the commit,
 * including a flatpak that has an extension marked autodownload. Pull the update
 * and kill the network connection by deleting the remote repositories. Simulate
 * a reboot by running eos-updater-flatpak-installer. Both the incoming package
 * and the extension marked autodownload should be installed. (in this case,
 * dependency resolution will deploy the extension during the fetch
 * phase such that it will appear to be installed after a reboot). */
static void
test_update_deploy_dependency_autodownload_extension_flatpaks_on_reboot_no_network (EosUpdaterFixture *fixture,
                                                                                    gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
...
/* Insert a list of flatpaks to automatically uninstall on the commit
 * with one of them that has an extension marked autodelete. Both are
 * installed by default but only mark the app flatpak for uninstallation.
 * Download the update and simulate a reboot by running
 * eos-updater-flatpak-installer. Both the app flatpak and its autodelete
 * extension should no longer be installed */
static void
test_update_uninstall_flatpaks_on_reboot_autodelete_extension (EosUpdaterFixture *fixture,
                                                               gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
  ...
  g_autoptr(GPtrArray) extension_infos =
    g_ptr_array_new_with_free_func ((GDestroyNotify) flatpak_extension_point_info_free);
  g_auto(GStrv) predeployed_flatpaks = NULL;
  g_auto(GStrv) deployed_flatpaks = NULL;
  g_autofree gchar *deployment_repo_relative_path = g_build_filename ("sysroot", "ostree", "repo", NULL);
  g_autofree gchar *deployment_csum = NULL;
  g_autofree gchar *refspec = concat_refspec (default_remote_name, default_ref);
  ...
  /* Commit number 1 will remove the app flatpak
   */
  autoinstall_flatpaks_files (1,
                              flatpaks_to_install,
                              1,
  ...
  /* Set up an autodelete extension point for the preinstalled app */
  ...
  /* Set up runtime, app and extension, all of which will be installed by default */
  g_ptr_array_add (flatpak_install_infos,
                   flatpak_install_info_new (FLATPAK_INSTALL_INFO_TYPE_RUNTIME,
                                             "org.test.Runtime",
                                             "stable",
  ...
  /* Quick sanity check that the app and extension are installed */
  predeployed_flatpaks = eos_test_get_installed_flatpaks (updater_directory, &error);
  g_assert_no_error (error);

  g_assert_true (g_strv_contains ((const gchar * const *) predeployed_flatpaks,
                                  "org.test.Test"));
  g_assert_true (g_strv_contains ((const gchar * const *) predeployed_flatpaks,
                                  "org.test.Test.Extension"));
  ...
  /* Extension should not be installed here, nor should the app */
  deployed_flatpaks = eos_test_get_installed_flatpaks (updater_directory, &error);
  g_assert_no_error (error);
...
/* Insert a list of flatpaks to automatically uninstall on the commit
 * with one of them that has an extension not marked autodelete. Both are
 * installed by default but only mark the app flatpak for uninstallation.
 * Download the update and simulate a reboot by running
 * eos-updater-flatpak-installer. The app flatpak should not be installed,
 * but the non-autodelete extension should still be installed. */
static void
test_update_uninstall_flatpaks_on_reboot_no_autodelete_extension (EosUpdaterFixture *fixture,
                                                                  gconstpointer      user_data)
{
  g_auto(EtcData) real_data = { NULL, };
  EtcData *data = &real_data;
  ...
  /* Extension should be installed here but app should not be */
  deployed_flatpaks = eos_test_get_installed_flatpaks (updater_directory, &error);
  g_assert_no_error (error);
      ...
      char **argv)
{
  ...
  eos_test_add ("/updater/install-flatpaks-deploy-dependency-runtime-on-reboot-no-network", NULL, test_update_deploy_dependency_runtime_flatpaks_on_reboot_no_network);
  eos_test_add ("/updater/install-flatpaks-deploy-dependency-autodownload-extension-on-reboot-no-network", NULL, test_update_deploy_dependency_autodownload_extension_flatpaks_on_reboot_no_network);
  ...
  eos_test_add ("/updater/uninstall-flatpaks-on-reboot-autodelete-extension", NULL, test_update_uninstall_flatpaks_on_reboot_autodelete_extension);
  eos_test_add ("/updater/uninstall-flatpaks-on-reboot-no-autodelete-extension", NULL, test_update_uninstall_flatpaks_on_reboot_no_autodelete_extension);
