...
#include <eos-updater-flatpak-installer/installer.h>
...
static GHashTable *
sample_flatpak_ref_actions_of_type (const gchar * const                      *flatpaks_to_install,
                                    EosUpdaterUtilFlatpakRemoteRefActionType  action_type)
{
  g_autoptr(GPtrArray) actions = g_ptr_array_new_full (1, (GDestroyNotify) flatpak_remote_ref_action_unref);
  ...
  for (i = 0; flatpaks_to_install[i] != NULL; ++i)
    {
      g_autoptr(FlatpakRef) ref = g_object_new (FLATPAK_TYPE_REF,
                                                ...
                                                NULL);
      g_autoptr(FlatpakLocationRef) location_ref = flatpak_location_ref_new (ref,
                                                                             "test-repo",
                                                                             NULL);
      g_autoptr(FlatpakRemoteRefAction) ref_action = flatpak_remote_ref_action_new (action_type,
                                                                                    location_ref,
                                                                                    i + 1);

      g_ptr_array_add (actions, flatpak_remote_ref_action_ref (ref_action));
...
static GHashTable *
sample_flatpak_ref_actions (const gchar * const *flatpaks_to_install)
{
  return sample_flatpak_ref_actions_of_type (flatpaks_to_install,
                                       ...
                                       gconstpointer              user G_GNUC_UNUSED)
{
  ...
  g_autoptr(GHashTable) actions = sample_flatpak_ref_actions (flatpaks_to_install);
  ...
  eos_updater_flatpak_installer_apply_flatpak_ref_actions (installation,
                                                           state_counter_path,
                                                           actions,
                                                       ...
                                                       gconstpointer              user G_GNUC_UNUSED)
{
   ...
   * up to date */
  eos_updater_flatpak_installer_check_ref_actions_applied (installation,
                                                           state_counter_path,
                                                           actions,
                                                           &error);
  g_assert_no_error (error);
                                                                ...
                                                                gconstpointer              user G_GNUC_UNUSED)
{
  ...
  g_autoptr(GHashTable) actions = sample_flatpak_ref_actions_of_type (flatpaks_to_install,
